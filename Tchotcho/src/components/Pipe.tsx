...
function PipeComponent({ x, topHeight, gapEnd, screenHeight }: PipeProps) {
  // الأنبوب السفلي يمتد حتى أسفل الشاشة كاملاً (الأرض بـ zIndex:15 تغطي قاعدته)
  const bottomHeight = Math.max(screenHeight - gapEnd, 0);

  return (
    <div className="absolute top-0" style={{ left: x, width: W, height: screenHeight, willChange: 'left', zIndex: 10 }}>
      {/* أنبوب علوي */}
      {topHeight > 0 && (
